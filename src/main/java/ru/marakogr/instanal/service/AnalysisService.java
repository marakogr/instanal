...
public class AnalysisService {
  ...
  public void analyzeAllFriends(SuperUser owner) {
    ...
    for (FriendRelation relation : relations) {
      ...
      var friendReelsMids =
          messageRepository.findAllByHasReelTrueAndSenderId(friendId).stream()
              .map(MessageEntity::getPrimaryMid)
              .toList();
      double myReactionsOnFriendReels = 0;
      if (!friendReelsMids.isEmpty()) {
        myReactionsOnFriendReels =
            messageRepository.countByHasReactionTrueAndReactionSenderIdAndPrimaryMidIn(
                ownerId, friendReelsMids);
      }
      long reelsFromMe = messageRepository.countByHasReelTrueAndSenderId(ownerId);
      var myReelsMids =
          messageRepository.findAllByHasReelTrueAndSenderId(ownerId).stream()
              .map(MessageEntity::getPrimaryMid)
              .collect(Collectors.toList());
      double hisReactionsOnMyReels = 0;
      if (!myReelsMids.isEmpty()) {
        hisReactionsOnMyReels =
            messageRepository.countByHasReactionTrueAndReactionSenderIdAndPrimaryMidIn(
                friendId, myReelsMids);
      }
      double finalRating =
          getFinalRating(
              reelsFromFriend, myReactionsOnFriendReels, reelsFromMe, hisReactionsOnMyReels);
      relation.setRating(finalRating);
      friendRelationRepository.save(relation);
    }
  }

  private static double getFinalRating(
      long reelsFromFriend,
