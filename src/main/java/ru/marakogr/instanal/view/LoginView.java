  ...
  public LoginView(SuperUserService service) {
    ...
    submit.addClickListener(
        e -> {
          ...
          if (currentMode[0] == Mode.LOGIN) {
            loggedUser = service.findByInstagram(instagram.getValue()).orElse(null);
            if (loggedUser == null) {
    ...
    UI.getCurrent()
        ...
        .executeJs(
            "var stored = localStorage.getItem('superUserInstagram');"
                + "if(stored) $0.$server.loginFromLocalStorage(stored);",
            getElement());
  }
